...
from core_starflow_refine import multiScaleLoss, refineLoss, diss
...
def main():
    ...
    '''CREATE DIR'''
    experiment_dir = Path('./Evaluate_experiments_refine/')
    experiment_dir.mkdir(exist_ok=True)
    file_dir = Path(str(experiment_dir) + '/%sSTARFlow-'%args.model_name + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
    ...
    for i, data in tqdm(enumerate(val_loader, 0), total=len(val_loader), smoothing=0.9):
        ...
        with torch.no_grad(): 
            ...
            disst = diss(gt_flow=flow, pre_flow=refineFlow, pc1=pos1, pc2=pos2)
                        
            flow_loss = refineLoss(refineFlow, flow)
            
            loss = flow_loss
            full_flow = refineFlow.permute(0, 2, 1)
        ...
        total_disst += disst.cpu().data * args.batch_size
    ...
    str_out2 = '%s %s mean loss: %f'%(blue('Evaluate'), blue('eval_flow_loss'), train_flow_loss)
    print(str_out2)
    ...
    train_disst_loss = total_disst / total_seen
    str_out4 = '%s %s mean loss: %f'%(blue('Evaluate'), blue('eval_diss_loss'), train_disst_loss)
    print(str_out4)
