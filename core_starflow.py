...
class STARFlow(nn.Module):
    ...
    def forward(self, xyz1, xyz2, color1, color2):
        ...
        all2allCost, all2allMap = self.all2allcv(pc1_l4, pc2_l4, feat1_l4, feat2_l4)
        ...
        return flows, fps_pc1_idxs, fps_pc2_idxs, pc1, pc2, feat1_new_l0, feat2_new_l0 # , all2allMap
...
# def featSimiLoss(gt_flow, pc1, pc2, feat1, feat2, n=9):
#     """
#     Input:
#         pc1: B N 3
#         pc2: B M 3
...
#     loss = scale * threshed_neighbor_simi.abs().sum(dim=[1,2]).mean() #[0. , 1.+threhold] * scale
    
#     return loss

def localFlowSmoothoss(gt_flow, pre_flow, pc1):
    """
    Input:
        gt_flow: B N 3
        pre_flow B 3 N
    ...
    grouped_pc1_flow_gt = index_points_group(flow_gt, kidx) # B N n C=3
    grouped_flow_diff_gt = grouped_pc1_flow_gt - flow_gt.unsqueeze(2) # B N n C=3
    
    grouped_pc1_flow_pre = index_points_group(flow_pre, kidx) # B N n C=3
    grouped_flow_diff_pre = grouped_pc1_flow_pre - flow_pre.unsqueeze(2) # B N n C=3
    loss_gt = torch.norm(grouped_flow_diff_gt, p=2, dim=-1, keepdim=False) # B N n
    # radius zero
    loss_gt = torch.mul(loss_gt, support)
    
    loss_pre = torch.norm(grouped_flow_diff_pre, p=2, dim=-1, keepdim=False) # B N n
    # radius zero
    loss_pre = torch.mul(loss_pre, support)
    
    loss = torch.norm(loss_gt - loss_pre, p=1, dim=-1, keepdim=False).mean() # B N 1
    
    return loss*scale

def featSimiLossBall(gt_flow, pc1, pc2, feat1, feat2):
    """
    Input:
        pc1: B N 3
        pc2: B M 3
    ...
    loss = scale * simi_thresh_mask.abs().sum(dim=[2]).mean()
    
    return loss

# for evaluate to look statistic   
def diss(gt_flow, pre_flow, pc1, pc2):
    ...
    loss = torch.norm(grouped_feat_diff_l12, p=2, dim=-1, keepdim=False) # B N n
...
if __name__ == '__main__':
    ...
    average_inference_time = timer / 100.0
    print("Average Inference Time: %.5f seconds" % average_inference_time)
