...
from core_starflow_refine import refineModule
from core_starflow_refine import multiScaleLoss, featSimiLoss, refineLoss, localConsisLoss
...
def main():
    ...
    '''CREATE DIR'''
    experiment_dir = Path('./experiment_refine/')
    ...
    if args.pretrain is not None:
        ...
        print('Error! Please Load The Pretrain Model')
        logger.info('Error! Please Load The Pretrain Model')
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            ...
            refineFlow = model(pos1, pos2, norm1, norm2)
            
            refineFlowLoss = refineLoss(refineFlow, flow)
            
            # local loss
            localC_loss = localConsisLoss(flow, refineFlow, pos1)
                       
            loss = refineFlowLoss + localC_loss
            ...
            total_flow_loss += refineFlowLoss.cpu().data * args.batch_size
            total_localC_loss += localC_loss.cpu().data * args.batch_size
        scheduler.step()

        train_flow_loss = total_flow_loss / total_seen
        str_out2 = 'EPOCH %d %s mean loss: %f'%(epoch, blue('train_flow_loss'), train_flow_loss)
        print(str_out2)
        ...
        train_loss = total_localC_loss / total_seen
        str_out1 = 'EPOCH %d %s mean local csc loss: %f'%(epoch, blue('train'), train_loss)
        print(str_out1)
        logger.info(str_out1)
          
        eval_epe3d, eval_loss, localCs_loss = eval_sceneflow(model.eval(), val_loader)
        
        str_out = 'EPOCH %d %s mean epe3d: %f  mean eval loss: %f'%(epoch, blue('eval'), eval_epe3d, eval_loss)
        print(str_out)
        logger.info(str_out)
        
        str_out1 = 'EPOCH %d %s mean local csc loss: %f'%(epoch, blue('train'), localCs_loss)
        print(str_out1)
        logger.info(str_out1)
...
def eval_sceneflow(model, loader):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            refineFlowLoss = refineFlowLoss + localC_loss

            epe3d = torch.norm(refineFlow.permute(0, 2, 1) - flow, dim = 2).mean()

        metrics['epe3d_loss'].append(epe3d.cpu().data.numpy())
        metrics['eval_loss'].append(refineFlowLoss.cpu().data.numpy())
        metrics['localCs_loss'].append(localC_loss.cpu().data.numpy())

    mean_epe3d = np.mean(metrics['epe3d_loss'])
    mean_eval = np.mean(metrics['eval_loss'])
    localCS_loss = np.mean(metrics['localCs_loss'])

    return mean_epe3d, mean_eval, localCS_loss
